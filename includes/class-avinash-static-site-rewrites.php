...
class Avinash_Static_Site_Rewrites {
	...
	public function rules(): array {
		...
		return array(
			...
			'RewriteRule ^$ ' . $cache_path . '/index.html [E=PSSC_HIT:1,L]',
			'RewriteCond %{REQUEST_METHOD} ^(GET|HEAD)$ [NC]',
			'RewriteCond %{QUERY_STRING} ^$',
			'RewriteCond %{HTTP_COOKIE} !(wordpress_logged_in_|comment_author_|wp-postpass_) [NC]',
			'RewriteCond %{REQUEST_URI} !^/wp-admin/ [NC]',
			...
			'RewriteRule ^(.+?)/?$ ' . $cache_path . '/$1/index.html [E=PSSC_HIT:1,L]',
			'</IfModule>',
			'<IfModule mod_headers.c>',
			'Header set X-Avinash-Static-Cache "STATIC-HIT" env=PSSC_HIT',
			'Header set X-Avinash-Static-Cache "STATIC-HIT" env=REDIRECT_PSSC_HIT',
			'</IfModule>',
		);
	}
